...
const CampaignEditPage = () => {
    ...
    const { data: campaignData, isLoading: isFetching } = useGetSingleCampaignQuery(id);
    ...
    useEffect(() => {
        if (campaignData?.data) {
            form.reset({
                ...
                subject: campaignData.data.subject,
                content: campaignData.data.content,
                target: campaignData.data.target || [],
                manualEmails: campaignData.data.manualEmails || [],
                excludedEmails: campaignData.data.excludedEmails || [],
            });
        }
    }, [campaignData, form]);
    ...
    const onSubmit = async (values: CampaignFormValues) => {
        try {
            await updateCampaign({ id, ...values }).unwrap();
            toast.success("Campaign updated successfully");
            router.push("/dashboard/campaigns");
        } catch (err: any) {
            toast.error(err?.data?.message || "Failed to update campaign");
        }
    };
            ...
            <header className="mb-10 flex flex-col md:flex-row items-center gap-6 border-b pb-8">
                <div className="flex items-center gap-4 w-full">
                    ...
                    <div className="flex-grow">
                        <h1 className="text-3xl font-black text-gray-900 tracking-tight">Edit Campaign</h1>
                        <p className="text-sm font-medium text-gray-500">Modify your draft and update your audience</p>
                    </div>
                </div>
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                >
                                    {isUpdating ? (
                                        ...
                                        <><Save className="w-5 h-5 mr-3" /> Update Campaign</>
                                    )}
                                </Button>
                                <Button
                                    type="button"
