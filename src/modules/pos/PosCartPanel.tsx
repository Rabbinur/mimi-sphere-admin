...
import { useLazyLookupCustomerQuery } from "@/components/Redux/RTK/posApi";
...
export function PosCartPanel({
  ...
  const [triggerLookupQuery, { isFetching: isLookingUp }] = useLazyLookupCustomerQuery();

  const handleLookup = async (phoneToSearch?: string) => {
    const raw = (phoneToSearch || phoneInput).trim();
    ...
    try {
      const res = await triggerLookupQuery(clean).unwrap();
      const cust = res?.data || res;

      if (cust && cust.phone) {
        onSelectCustomer(cust);
        setIsNewCustomerOpen(false);
        if (cust.membership_tier !== "Regular") {
          toast.success(`${cust.membership_tier} Member detected! ${cust.discount_percent}% discount applied.`);
        } else {
          toast.info(`Customer ${cust.name} attached.`);
        }
      } else {
  ...
  return (
    <div className="w-full lg:w-80 xl:w-92 2xl:w-96 bg-white h-full flex flex-col justify-between border-l border-slate-200 select-none">
      {/* Panel Header */}
      <div className="px-3.5 py-3 border-b border-slate-200 flex items-center justify-between shrink-0 bg-white">
        ...
        {cartItems.length > 0 && (
          <button
            type="button"
            onClick={onClearCart}
            className="px-2.5 py-1 text-[10px] font-bold text-rose-500 hover:bg-rose-50 rounded-lg transition-all flex items-center gap-1 cursor-pointer border border-rose-100"
          >
      ...
      {/* Customer / Membership Quick Bar */}
      <div className="px-3 py-2.5 bg-slate-50 border-b border-slate-200">
        {!customer ? (
          <div className="space-y-2">
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl px-3 py-2 shadow-xs focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-100 transition-all">
              <Phone className="w-3.5 h-3.5 text-slate-400 shrink-0" />
              <input
                type="tel"
                placeholder="Member phone number (01...)"
                value={phoneInput}
                onChange={(e) => {
                  const val = e.target.value;
                  setPhoneInput(val);
                  if (val.trim().replace(/[\s-]/g, "").length === 11) {
                    handleLookup(val);
                  }
                }}
                onKeyDown={(e) => {
                ...
                className="flex-1 text-xs font-medium text-slate-800 placeholder:text-slate-400 outline-none bg-transparent"
              />
              <button
                type="button"
                onClick={() => handleLookup()}
                disabled={isLookingUp || !phoneInput.trim()}
                className="px-2.5 py-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-lg text-[10px] font-black transition-all cursor-pointer shrink-0 flex items-center gap-1"
              >
            ...
            {/* If searched and not found, show quick register */}
            {isNewCustomerOpen && (
              <div className="p-2 bg-amber-50 border border-amber-200 rounded-lg space-y-1.5 animate-in fade-in duration-150">
                ...
                <div className="flex items-center gap-1.5">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        const temp: PosCustomer = {
                          name: newCustomerName.trim() || "Walk-in Customer",
                    ...
                    className="flex-1 px-2 py-1 bg-white border border-amber-300 rounded text-xs text-slate-800 outline-none focus:border-amber-500"
                  />
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-2.5 py-1 bg-amber-600 hover:bg-amber-700 text-white rounded text-xs font-bold cursor-pointer shrink-0"
                  >
                    Attach
                  </button>
                </div>
          ...
          /* Active Customer & Tier Badge Card */
          <div className={`p-2 rounded-xl border flex items-center justify-between transition-all ${
            ...
            <div className="min-w-0 pr-1">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                <span className="font-black text-xs truncate max-w-[110px]">{customer.name}</span>
                <span className={`px-1.5 py-0.5 text-[9px] font-black rounded ${
                  ...
                  {customer.membership_tier === "Regular"
                    ? "Regular"
                    : `${customer.membership_tier} (${customer.discount_percent}% Off)`}
                </span>
              </div>
              <div className="text-[10px] text-slate-600 mt-0.5 flex items-center gap-1.5 flex-wrap font-mono">
                <span>{customer.phone}</span>
                <span>•</span>
                <span>Spent: ৳{customer.total_spent.toLocaleString()}</span>
                {customer.membership_tier === "Regular" && customer.total_spent < 1000 && (
                  <span className="text-blue-700 font-bold font-sans">
                    (৳{(1000 - customer.total_spent).toLocaleString()} to Silver)
                  </span>
                )}
                {customer.membership_tier === "Silver" && customer.total_spent < 3500 && (
                  <span className="text-amber-800 font-bold font-sans">
                    (৳{(3500 - customer.total_spent).toLocaleString()} to Gold)
                  </span>
                )}
              </div>
            </div>
            ...
            <button
              ...
              className="p-1 hover:bg-black/10 rounded-lg text-slate-500 hover:text-slate-900 transition-colors cursor-pointer shrink-0"
            >
